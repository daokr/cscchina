...
function pageInit()
{
	...
	var tsplugin={
		...
		tsImg:{c:'tsImg',t:'插入图片',e:function(){
			var _this=this;
			 ...
			 _this.showIframeModal('选择图片',siteUrl+'index.php?g=admin&m=images&a=imgupload&type='+type+'&typeid='+typeid,function(v){
				  v = '<img src="'+v+'" />'
				  _this.pasteHTML(v,false); },400,300);
		}},
		...
		tsVideo:{c:'tsVideo',t:'插入视频地址',h:1,e:function(){
			var _this=this;
			_this.saveBookmark();
			var htmlCode=$('<div>网站的有：土豆网、优酷网、酷6网、56网、的视频。</div><div><img src="" id="urlImg" style="display:none"/></div><div>输入视频播放页地址: <input type="text" id="xheVidoeUrl" value="" class="xheText" /></div><div>标题: <input type="text" id="xheVidoeTitle" value="" class="xheText" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="解析" /></div>');
