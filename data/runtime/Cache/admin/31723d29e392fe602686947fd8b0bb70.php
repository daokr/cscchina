...
<form method="POST" action="<?php echo U('content/editarticle',array('id'=>$strContent[id]));?>" id="formMini" onsubmit="return miniSubmit(this)">
<table cellpadding="0" cellspacing="0">
	<tr>
		<th>标题：</th>
		<td><input name="title" value="<?php echo ($strContent[title]); ?>" style="width:400px"/></td>
	</tr> 
	<tr>
		<th>所属年份：</th>
		<td>
            <select name="yearid">
            <?php if(is_array($years)): foreach($years as $key=>$item): if($item[yearid] == $strContent[yearid]): ?><option value="<?php echo ($item[yearid]); ?>" selected="selected"><?php echo ($item[yearname]); ?></option>
                <?php else: ?>
                <option value="<?php echo ($item[yearid]); ?>"><?php echo ($item[yearname]); ?></option><?php endif; endforeach; endif; ?>    
            </select>
        </td>
	</tr>     
	<tr>
		...
		<td>
            <select name="cateid">
            <?php if(is_array($cates)): foreach($cates as $key=>$item): if($item[cateid] == $strContent[cateid]): ?><option value="<?php echo ($item[cateid]); ?>" selected="selected"><?php echo ($item[catename]); ?></option>
                <?php else: ?>
	...
	<tr>
		<th>内容：</th>
		<td>
<textarea style="width:100%;" id="editor_mini" name="content" class="txt"   placeholder="请填写内容"><?php echo ($strContent[content]); ?></textarea>        
        </td>
	</tr> 
	<tr>
		<th>作者：</th>
		<td><input name="newsauthor" value="<?php echo ($strContent[newsauthor]); ?>" style="width:300px"/></td>
	</tr>               
</table>
...
<script>
	var type = 'awards';
	var typeid = '<?php echo ($strContent[id]); ?>';
</script>
